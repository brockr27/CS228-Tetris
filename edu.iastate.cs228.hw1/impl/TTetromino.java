...
/**
 * A class which constructs a TTetromino piece.
 * @author Brock Rikkers
 */
public class TTetromino extends Tetromino implements IPolyomino
{
	/**
	...
	public TTetromino(Point p, boolean magicState)
	{
		position = new Point(p);
		
		cells = new Cell[4];
		cells[0] = new Cell(new Block(COLOR, false), new Point(position.x, position.y-1));
		cells[1] = new Cell(new Block(COLOR, false), new Point(position.x-1, position.y));
		cells[2] = new Cell(new Block(COLOR, magicState), position);
		cells[3] = new Cell(new Block(COLOR, false), new Point(position.x+1, position.y));
	...
	{
		IPolyomino clone = new TTetromino(position, false);
