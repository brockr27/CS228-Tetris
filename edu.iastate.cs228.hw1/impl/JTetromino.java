...
/**
 * A class which constructs a JTetromino piece.
 * @author Brock Rikkers
 */
public class JTetromino extends Tetromino implements IPolyomino
{
	/**
	...
	public JTetromino(Point p, boolean magicState)
	{
		position = new Point(p);
		
		cells = new Cell[4];
		cells[0] = new Cell(new Block(COLOR, magicState), new Point(position.x-1, position.y));
		cells[1] = new Cell(new Block(COLOR, false), new Point(position.x-1, position.y+1));
	...
	{
		IPolyomino clone = new JTetromino(position, false);
